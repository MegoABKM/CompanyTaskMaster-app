...
{
    // THE FIX: Check if the key exists before trying to access it.
    // If it doesn't exist, return null.
    if (isset($_POST[$requestname])) {
        // If it exists, sanitize it and return it.
        return htmlspecialchars(strip_tags($_POST[$requestname]));
    } else {
        // If it does not exist, do not cause a warning. Just return null.
        return null;
    }
}
...
// ==========================================================
function insertDataAndReturnId($table, $data, $json = true)
{
    global $con;
    $fields = implode(',', array_keys($data));
    $placeholders = ':' . implode(', :', array_keys($data));
    ...
    if ($json == true) {
        if ($count > 0) {
            ...
            echo json_encode(array("status" => "success", "data" => array("id" => $con->lastInsertId())));
        } else {
            echo json_encode(array("status" => "failure"));
        }
    }
    return $con->lastInsertId(); // Also return for non-json usage
}
// ==========================================================
