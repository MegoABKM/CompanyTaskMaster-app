...
class SqlDb {
  ...
  Future<Database> _initializeDb() async {
    ...
    Database myDb = await openDatabase(
      path,
      version: 3, // Incremented version for removing stopwatch columns
      onCreate: _onCreate,
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 3) {
      // Drop stopwatch columns if they exist
      await db.execute(
    ...
    print(
        "Upgraded database from version $oldVersion to $newVersion==============");
  }
